...
class UniqueObjectsLocalization(Node):
    ...
    def __init__(self):
        ...
        MAX_DISPLAY_RANGE = 50.0  # LiDar max ragne in meters

        self.scale = (self.bev_height / 2) / MAX_DISPLAY_RANGE
        ...
        self.pipeline_3d.MIN_OBJECT_POINTS = self.MIN_OBJECT_POINTS
        ...
        self.pipeline_object_association.association_distance = 7
    ...
    def synchronized_callback(
        ...
        if self.previous_frame is not None:
            ...
            if current_frame < self.previous_frame:
                ...
                self.get_logger().info(
                    "Replay restarted. Resetting AB3DMOT tracker."
                )

                self.ab3dmot_tracker.reset()
        ...
        unique_objects = self.pipeline_object_association.associate(world_objects)
        
        # bev = self.pipeline_display.draw_unique_objects(unique_objects)

        tracks = self.ab3dmot_tracker.update(unique_objects)

        bev = self.pipeline_display.draw_tracks(tracks)

        
        
        # bev = self.pipeline_display.draw_world_objects(world_objects)
        # bev = self.pipeline_display.draw_association_bev(bev, unique_objects)      
    ...
    def stitch_images(
        ...
        # Split rear image into left/right halves
        h, w = rear.shape[:2]
